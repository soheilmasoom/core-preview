# Generated by Django 4.1.3 on 2024-08-18 12:17

from django.db import migrations

def add_user_to_payment_request(apps, schema_editor):
    PaymentRequest = apps.get_model('financial', 'PaymentRequest')
    Payment = apps.get_model('financial', 'Payment')

    batch_size = 1000
    offset = 0

    while True:
        payments_list = list(
            Payment.objects.select_related('paymentrequest')
            .filter()
            .order_by('id')[offset:offset + batch_size]
        )

        if not payments_list:
            break

        payment_requests_to_update = []

        for payment_obj in payments_list:
            payment_obj.payment_request.user = payment_obj.user
            payment_requests_to_update.append(payment_obj.payment_request)

        PaymentRequest.objects.bulk_update(payment_requests_to_update, ['user'])

        offset += batch_size

class Migration(migrations.Migration):

    dependencies = [
        ('financial', '0138_paymentrequest_user_alter_paymentrequest_bank_card'),
    ]

    operations = [
        migrations.RunPython(add_user_to_payment_request, migrations.RunPython.noop)
    ]
